...
def count_sampels(names_in, names_out, kwtree, new_labels_idx):
    ...
    for name_idx in range(len(names_in)):
        ...
        result_mtx = np.zeros((len(cleared_dff), 51), dtype=bool)  # list()
        cnt = 0
...
def main():
    ...
    for key in attrPred.idx_label.keys():
        attrPred.idx_label[key] = attrPred.idx_label[key].replace("+", " ")
    # print(attrPred.idx_label)
    kwtree, new_labels_idx = create_search_tree(attrPred.idx_label)

    with open("/data/felix/new_bachelor/awa2/dataset_analysis/laion400M_search/class_names/aho_corasick_class_repr_only_ending.pickle", "wb") as act:
        pickle.dump(kwtree, act, protocol=pickle.HIGHEST_PROTOCOL)
    act.close()
    with open("/data/felix/new_bachelor/awa2/dataset_analysis/laion400M_search/class_names/new_labels_idx_only_ending.pickle", "wb") as act:
    ...
    occurrence = get_occurrence(names_out, attrPred.idx_label)
    with open("/home/felix/new_bachelor/awa2/dataset_analysis/laion400M_search/class_names/absolute_occurence.pickle", "wb") as act:
...
def save_attr_occ_mapping():
    ...
    for key in attrPred.idx_label.keys():
        attrPred.idx_label[key] = attrPred.idx_label[key].replace("+", " ")
    idx_label = attrPred.idx_label

    with open("/home/felix/new_bachelor/awa2/dataset_analysis/laion400M_search/class_names/absolute_occurence.pickle", "rb") as act:
    ...
    with open("/home/felix/new_bachelor/awa2/dataset_analysis/laion400M_search/class_names/attr_idx_occurence.pickle", "wb") as act:
        pickle.dump(res_mapping, act, protocol=pickle.HIGHEST_PROTOCOL)
    act.close()
    with open("/home/felix/new_bachelor/awa2/dataset_analysis/laion400M_search/class_names/attr_readable_occurence.pickle", "wb") as act:
